...
(async () => {
  ...
  try {
    ...
    // Navigate directly to room.examgoal.com/ (Root domain)
    console.log("Navigating to room.examgoal.com/ (Root)...");
    apis.length = 0;
    ...
    console.log("Root Page Preview (first 1000 chars):");
    ...
    apis.forEach((api, idx) => {
      if (api.url.includes('/api/v1/')) {
        ...
        // If it looks like subjects list API, print first 200 chars
    ...
    // Dump page content
    fs.writeFileSync('page_root_adv.html', await page.content());
