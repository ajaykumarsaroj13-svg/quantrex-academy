...
(async () => {
  ...
  try {
    ...
    // Navigate directly to room.examgoal.com/books
    console.log("Navigating to room.examgoal.com/books...");
    apis.length = 0;
    await page.goto('https://room.examgoal.com/books', { waitUntil: 'networkidle2' });
    await new Promise(r => setTimeout(r, 6000));
    
    console.log("Current URL:", page.url());
    
    const body = await page.evaluate(() => document.body.innerText);
    console.log("Books Page Preview (first 1000 chars):");
    ...
    apis.forEach((api, idx) => {
      if (api.url.includes('/api/v1/')) {
        ...
        if (api.url.includes('book') || api.url.includes('meta')) {
    ...
    // Dump page content
    fs.writeFileSync('page_books.html', await page.content());
