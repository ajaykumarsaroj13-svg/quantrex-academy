...
// Add marquee animation classes
newToppersSection = newToppersSection.replace('<div className="grid grid-cols-1 md:grid-cols-3 gap-8">', '<div className="w-full overflow-hidden"><div className="animate-marquee gap-8 py-4 px-4">');
// Change map to have duplicated items for infinite scroll
newToppersSection = newToppersSection.replace('{(toppers || []).map((t, idx) => (', '{[...(toppers || []), ...(toppers || []), ...(toppers || []), ...(toppers || [])].map((t, idx) => (');
// Update card sizes for marquee
...
// Add missing closing div for the marquee wrapper
