...
export default function ExamGoalPracticeInterface({ pyqData, topic, customQuestions, practiceMode, onProgressUpdate, onClose, isLight, onToggleTheme, bookmarkGroups = [], addBookmarkGroup = () => {}, progress = {} }) {
  ...
  // Helper to fix scattered MathJax
            ...
            {(isAnswerChecked || isTestSubmitted) && (
               <div className="mt-8">
                 ...
                 <TeacherSolution html={currentQuestion.solution} isLight={isLight} correctOptionLabel={(() => {
                   ...
                   if (isNumerical) return q.correctAnswer ? String(q.correctAnswer) : '';
                   if (q.correctOptionsArray && q.correctOptionsArray.length > 0) return q.correctOptionsArray.map(i => String.fromCharCode(65 + parseInt(i))).join(', ');
                   if (Array.isArray(q.correctOptionIndex)) return q.correctOptionIndex.map(i => String.fromCharCode(65 + i)).join(', ');
                   if (typeof q.correctOptionIndex === 'number') return String.fromCharCode(65 + q.correctOptionIndex);
