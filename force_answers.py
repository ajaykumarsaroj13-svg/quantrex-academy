...
# Always use the source of truth
data_path = r'E:\quantrexacademy\blackBookDataFull.json'
        ...
        for q in ch.get('questions', []):
            ...
            if ex_title and ex_title in ans_map[ch_id]:
                ...
                if q_num in ans_dict:
                    ...
                    if 'MCQ' in q.get('typeLabel', '').upper() or 'MULTIPLE' in q.get('typeLabel', '').upper():
                        opts = ['a','b','c','d']
                        q['correctOptionsArray'] = [opts.index(c) for c in correct_ans.lower() if c in opts]
                    elif 'SCQ' in q.get('typeLabel', '').upper() or 'SINGLE' in q.get('typeLabel', '').upper():
                        opts = ['a','b','c','d']
                        if correct_ans.lower() in opts:
                            q['correctOption'] = opts.index(correct_ans.lower())
                            # Also clear correctOptionsArray if it exists
                            if 'correctOptionsArray' in q:
                                del q['correctOptionsArray']
                    
                    # Ensure subjective/numerical have answerKeyStr
                    if 'SUBJECTIVE' in q.get('typeLabel', '').upper() or 'MATCHING' in q.get('typeLabel', '').upper() or 'NUMERICAL' in q.get('typeLabel', '').upper():
                        q['answerKeyStr'] = correct_ans
