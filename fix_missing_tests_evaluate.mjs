...
async function main() {
  ...
  if (TESTS.includes(target)) {
     TESTS = [target, ...TESTS.filter(t => t !== target)];
  }

  console.log(`=== Updating ${TESTS.length} untouched tests via evaluate fetch ===`);
    ...
    try {
      ...
      for (const ep of endpoints) {
        ...
        if (res && res.statusCode === 0 && res.data) {
          const data = res.data;
          qs = data.questions || (Array.isArray(data) ? data : null) || data.test?.questions || data.testSections?.flatMap(s => s.questions || []);
      ...
      if (qs && qs.length > 0) {
        ...
        successCount++;
        if (i % 10 === 0 || testId === target) console.log(`  [${i+1}/${TESTS.length}] Updated ${testId} - ${flatQ.length} qs`);
      } else {
        console.log(`  [${i+1}/${TESTS.length}] FAILED ${testId} - No data found in evaluate`);
        failedCount++;
      }
    } catch(e) {
