    ...
    <div className={`w-full min-h-screen ${isLight ? 'bg-gray-50' : 'bg-obsidian'} pb-20`}>
      ...
      <div className={`pt-24 pb-12 px-6 ${isLight ? 'bg-white border-b border-gray-200' : 'bg-cyberdark/50 border-b border-white/5'}`}>
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className={`text-3xl md:text-5xl font-black font-display ${isLight ? 'text-gray-900' : 'text-white'}`}>
            {bookData.title}
          </h1>
      ...
      <div className="max-w-4xl mx-auto px-6 mt-12">
        <h3 className={`text-xl font-bold mb-6 flex items-center gap-2 ${isLight ? 'text-gray-900' : 'text-white'}`}>
        ...
        <div className="space-y-4">
          {bookData.chapters.map((chapter) => (
            ...
            >
              <div>
                <h4 className={`text-lg font-bold ${isLight ? 'text-gray-900' : 'text-white'} group-hover:text-electric transition-colors`}>
                  {chapter.title}
                </h4>
