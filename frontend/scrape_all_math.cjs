...
const years = ['JEE Main 2019', 'JEE Main 2020', 'JEE Main 2021', 'JEE Main 2022', 'JEE Main 2023', 'JEE Main 2024'];
const shifts = ['Morning Shift', 'Evening Shift', 'Shift 1', 'Shift 2'];
...
async function scrapeMath() {
    console.log("🚀 STARTING MASSIVE MATH SCRAPE...");
    const mathUrl = 'https://questions.examside.com/past-years/jee/jee-main/mathematics';
    ...
    let allTests = [];
    try {
        if (fs.existsSync('./src/utils/testsData.json')) {
            allTests = JSON.parse(fs.readFileSync('./src/utils/testsData.json', 'utf8'));
    ...
    for (const chapter of uniqueChapters) {
        // Check if we already processed this chapter
        ...
        // Scraping concurrently in batches of 5 to avoid hammering the server too hard but still fast
        const BATCH_SIZE = 5;
        ...
        if (testQuestions.length > 0) {
            const cId = `math_chap_${chapIndex}`;
            allTests.push({
            ...
            fs.writeFileSync('./src/utils/testsData.json', JSON.stringify(allTests, null, 2));
            fs.writeFileSync('./src/utils/testsData2.json', JSON.stringify(allTests, null, 2));
        }
    }
    console.log("✅ FINISHED MASSIVE SCRAPE AND SAVED testsData.json");
}
