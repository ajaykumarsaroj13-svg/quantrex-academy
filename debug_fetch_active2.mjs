...
const testId = 'tst-19g61mo7g01e4';
...
async function main() {
  ...
  console.log('Fetching active test:', testId);
  
  const endpoints = [
    `/api/v1/test/user/test/${testId}?out_of_syllabus=false`,
  ...
  for (const ep of endpoints) {
    const res = await page.evaluate(async (origin, apiPath) => {
      try {
        ...
        return { error: true, status: r.status, text: await r.text() };
      } catch(e) {}
      return null;
    }, EXAM_ORIGIN, ep);
    
    console.log(`Endpoint ${ep} returned:`, typeof res === 'object' ? Object.keys(res || {}) : res);
    if (res && res.statusCode === 0 && res.data) {
      ...
      break;
       ...
       console.log(`Failed with status ${res.status}:`, res.text);
    }
  }
