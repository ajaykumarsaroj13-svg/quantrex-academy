...
def main():
    print("=== Restoring testsData to Vercel Blob (Dictionary Format) ===")
    ...
    formatted = {
        "mains": [t for t in test_series_metadata if t.get('examType') == 'JEE Main'],
        "advanced": [t for t in test_series_metadata if t.get('examType') == 'JEE Advanced'],
        "nda": [t for t in test_series_metadata if t.get('examType') == 'NDA']
    }
    
    if test_series_metadata:
        upload_to_blob('testsData.json', formatted)
        upload_to_blob('testSeries.json', formatted)
    else:
