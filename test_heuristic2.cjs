...
data.forEach(q => {
  ...
  if (q.options && q.options.length > 0 && !isMulti) {
    scq.push({year: q.year, title: q.title, sol: q.solution ? q.solution.substring(0, 150).replace(/\n/g, ' ') : ''});
  }
});
console.log("SCQ Count:", scq.length);
console.log(scq);
