...
async function fixNda() {
  ...
  for (const file of files) {
    ...
    for (const res of results) {
      ...
      for (const q of questions) {
        ...
        const chapterId = 'nda_' + subject + '_' + chapterSlug; // USING CONCATENATION TO AVOID ISSUES
        ...
        const qId = 'nda_' + (q.question_id || (Date.now() + Math.random().toString()));
        const year = String(q.year || '2024');
  ...
  console.log('Clearing old NDA data...');
  await PyqChapter.deleteMany({ id: { $regex: '^nda_' } });
  await Pyq.deleteMany({ id: { $regex: '^nda_' } });
  
  // also clear the bad literal ones just in case
  await PyqChapter.deleteMany({ id: { $regex: '.*\\$\\{subject\\}.*' } });
  await Pyq.deleteMany({ id: { $regex: '.*\\$\\{subject\\}.*' } });
  await Pyq.deleteMany({ id: { $regex: '.*\\$\\{q\\.year\\}.*' } });

  console.log('Inserting correct chapters...');
  await PyqChapter.insertMany(newChapters);

  console.log('Inserting correct questions...');
  ...
  console.log('Database fixing Done!');
  process.exit(0);
}

fixNda().catch(console.error);
