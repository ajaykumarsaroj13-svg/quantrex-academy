...
async function scrapeTest(page, tid, title, groupName, sectionName, policy, type) {
  ...
  try {
    ...
    // Wait for the "Please wait..." loading overlay to disappear
    console.log('  Waiting for page content to load...');
    await page.waitForFunction(() => !document.body.innerText.includes('Please wait...'), { timeout: 30000 });
    await delay(3000);
    ...
    if (currentUrl.includes('/analysis/')) {
      ...
      await page.waitForFunction(() => window.location.href.includes('/analysis/'), { timeout: 20000 });
      currentUrl = page.url();
      analysisId = currentUrl.split('/').pop();
      console.log(`  Redirected to analysis page. ID: ${analysisId}`);
    }

    // Now fetch the analysis JSON directly from the page context
...
async function main() {
  console.log('Launching browser in graphical (non-headless) mode to bypass ReCaptcha...');
