...
# Let's insert the typeFilter select exactly before the yearFilter select.
# Search for:
# <select 
#   value={yearFilter}
year_select_idx = content.find('value={yearFilter}')
if year_select_idx != -1:
    # go back to the <select
    select_start = content.rfind('<select', 0, year_select_idx)
    ...
    print("Failed to find yearFilter select")
